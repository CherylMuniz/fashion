...
class PingOptimizerPlugin extends PingOptimizer  
{
	...
	var $mpo_clear_log_duration = array(1 => 'Older than a Week', 2 => 'Older than 2 Week', 3 => 'Older than a Month');
	...
	function mbpoActivate() {
		...
		add_option('mpo_clear_log_duration', 1);
	...
	function mbpoOptionsPg() {
		...
		if ( intval($this->mpo_activate) == 0 ) { // First step of plugin registration
			...
			if ( $this->mbpo_request['pingnow'] ) {
				...
				update_option('mpo_clear_log_duration', $this->mbpo_request['clear_log_duration']);
				$msg = 'Options saved.';
			} else if ( $_GET['d'] == 'yes' ) {
	...
	function mbpoGetLogData() {
		...
		$this->__mpoDeleteLogPerDuration();
