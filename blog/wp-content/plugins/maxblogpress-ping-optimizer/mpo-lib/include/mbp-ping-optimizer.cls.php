...
class PingOptimizer {
	...
	*/
	...
	/**
	* Delete log as per the time duration set
	*/
	function __mpoDeleteLogPerDuration() {
		if (get_option('mpo_clear_log_duration') == 1) {
			$log_date =  date('Y-m-d H:i:s', strtotime('-1 week'));
		} else if (get_option('mpo_clear_log_duration') == 2) {
			$log_date =  date('Y-m-d H:i:s', strtotime('-2 week'));
		} else if (get_option('mpo_clear_log_duration') == 3) {
			$log_date =  date('Y-m-d H:i:s', strtotime('-1 month'));
		}
		
		$query_check_records = "SELECT COUNT(id) FROM $this->mpo_pinglog_tbl WHERE date_time <'$log_date'";
		$sql_check_records   = mysql_query($query_check_records);
		if (mysql_num_rows($sql_check_records) > 0) {
			$query = "DELETE FROM $this->mpo_pinglog_tbl WHERE date_time <= '$log_date'";
			mysql_query($query);
		}
	}	
