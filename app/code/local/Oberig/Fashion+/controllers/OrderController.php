...
class Oberig_Fashion_OrderController extends Mage_Adminhtml_Sales_OrderController
{
	public function addCommentAction()
	{
		if ($order = $this->_initOrder()) {
            try {
                ...
                $comment = trim(nl2br($data['comment']));
	...
	public function massAction()
	{
		...
		try {
			...
			$this->_getSession()->addSuccess($this->__('%s order(s) have been updated.', $updated));
		} catch (Exception $e) {
			$error = true;
			Mage::logException($e);
			$this->_getSession()->addError($this->__('%s of %s order(s) have been updated.', $updated, count($orderIds)));
		}
