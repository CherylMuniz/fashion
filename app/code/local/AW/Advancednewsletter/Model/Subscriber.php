 ...
 */class AW_Advancednewsletter_Model_Subscriber extends Mage_Core_Model_Abstract {
    ...
    protected function _afterSave() {
        if (Mage::helper('advancednewsletter')->extensionEnabled('AW_Points')) {
            if ($this->getData('add_points') && $this->getStatus() == self::STATUS_SUBSCRIBED && $this->getCustomer()->getId()) {
                $pointsForSubscription = Mage::helper('points/config')
                                ->getPointsForNewsletterSingup();

                Mage::getModel('points/api')->addTransaction(
                        $pointsForSubscription,
                        'an_customer_subscription',
                        $this->getCustomer(),
                        $this
                );
            }
    ...
    public function subscribe($email, $segments, $params = array()) {
        ...
        if ($this->getCustomer()->getId() && !isset($params['skip_customer_check'])) {
            ...
            if (!preg_match('/^1.3/', Mage::getVersion()) && !preg_match('/^1.4.0/', Mage::getVersion())) {
                $allowGuests = Mage::getStoreConfig(Mage_Newsletter_Model_Subscriber::XML_PATH_ALLOW_GUEST_SUBSCRIBE_FLAG);
                if (empty($allowGuests)) {
                    throw new Exception(Mage::helper('advancednewsletter')->__('Please, register to subscribe'));
                }
            }

            if (empty($email)) {
                throw new Exception(Mage::helper('advancednewsletter')->__('Email is empty'));
            }
        }

        $validator = new Zend_Validate_EmailAddress();
        if (!$validator->isValid($email)) {
            throw new Exception(Mage::helper('advancednewsletter')->__('Invalid email'));
        }
        ...
        if ($newSubscriber && $this->getStatus() == self::STATUS_SUBSCRIBED) {
            Mage::getModel('advancednewsletter/template')->sendFirstSubscribeMail($this);
        }
     ...
     * @return AW_Advancednewsletter_Model_Subscriber
     */
    protected function addSegments($segments) {
        if (empty($segments))
     ...
     * @return AW_Advancednewsletter_Model_Subscriber
     */
    protected function removeSegments($segments) {
        if (empty($segments))
            throw new Exception(Mage::helper('advancednewsletter')->__('Segments are not set'));
        if (!is_array($segments))
            $segments = explode(',', $segments);
    ...
    protected function getCustomer() {
        if (!$this->customer)
            $this->customer = Mage::getModel('customer/session')->getCustomer();
    ...
    public function getUnsubscriptionLink() {
        ...
        $key = urlencode($key);
        return Mage::app()->getStore($this->getStoreId())->getBaseUrl() . "advancednewsletter/index/unsubscribeAll/email/{$this->getEmail()}/key/{$key}";
    }
    ...
    public function needActivation() {
        return $this->getData('is_new') && Mage::getStoreConfig(self::NEED_TO_CONFIRM) && !$this->getCustomer()->getId();
    }
