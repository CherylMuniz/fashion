...
class AW_Advancednewsletter_Adminhtml_SubscriberController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function saveAction()
    {
        try
        {
            ...
            if ($data = $request->getPost())
            {
                ...
                if ($request->getParam('id')) $subscriber->load($request->getParam('id'));
                else $subscriber->setIsNew(true);
                if (!isset($data['segments_codes'])) $data['segments_codes'] = array();
                $subscriber->forceWrite($data);
                if ($subscriber->getStatus() == AW_Advancednewsletter_Model_Subscriber::STATUS_UNSUBSCRIBED) $subscriber->unsubscribeFromAll();
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('adminhtml')->__('Successfully saved'));
                if ($this->getRequest()->getParam('back')) return $this->_redirect('*/*/edit', array('id' => $subscriber->getId()));
            }
        }
        catch (Exception $ex)
        {
        ...
        try
        {
            ...
            if ($this->getSubscriberIds())
            {
                foreach($this->getSubscriberIds() as $subscriberId)
                {
                    ...
                    $subscriber->subscribe($subscriber->getEmail(), $segmentsToSubscribe);
                }
            }
            Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('adminhtml')->__('Successfully subscribed'));
        }
        catch (Exception $ex)
        {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('advancednewsletter')->__('Unable to find item to save'));
    ...
    public function massUnsubscribeAction()
    {
        try
        {
            ...
            {
                foreach($this->getSubscriberIds() as $subscriberId)
                    Mage::getModel('advancednewsletter/subscriber')->load($subscriberId)->unsubscribe($segmentsToUnsubscribe);
            }
            Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('adminhtml')->__('Successfully unsubscribed'));
        }
        catch (Exception $ex)
        {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('advancednewsletter')->__('Unable to find item to save'));
    ...
    public function massDeleteAction()
    {
        try
        {
            $segmentsToDelete = array($this->getRequest()->getParam('segment'));
            if ($this->getSubscriberIds())
            {
                foreach($this->getSubscriberIds() as $subscriberId)
    ...
    public function exportCsvAction()
    {
        ...
        if(version_compare(Mage::getVersion(),'1.3.3.0','<=')) { $content = $this->getLayout()->createBlock('advancednewsletter/adminhtml_subscriber_grid')->getCsv(); }
        else { $content = $this->getLayout()->createBlock('advancednewsletter/adminhtml_subscriber_grid')->getCsvFile(); }
    ...
    public function exportXmlAction()
    {
       ...
       if(version_compare(Mage::getVersion(),'1.3.3.0','<=')) { $content = $this->getLayout()->createBlock('advancednewsletter/adminhtml_subscriber_grid')->getXml(); }
       else { $content = $this->getLayout()->createBlock('advancednewsletter/adminhtml_subscriber_grid')->getExcelFile(); }
       $this->_prepareDownloadResponse($fileName, $content);
    }
