 ...
 */class AW_Advancednewsletter_Adminhtml_SegmentController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function saveAction()
    {
        ...
        if (!$request->getParam('code') || !$request->getParam('title') ||
            preg_match("/[<>, ]/", $request->getParam('code')) || preg_match("/[<>,]/", $request->getParam('title')))
        {
            Mage::getSingleton('adminhtml/session')
                ->addError(Mage::helper('advancednewsletter')->__('Segment code and title can not include \',<>\' and _ at the code beginning. Spaces mustn\'t be used in segment code'));
            return $this->_redirect('*/*/edit');
        }

        $idToRedirect = $request->getParam('id');
        try
        {
            $model = Mage::getModel('advancednewsletter/segment');
            if ($request->getParam('id')) $model->load($request->getParam('id'));
            $model
            ...
            if ($this->getRequest()->getParam('back')) return $this->_redirect('*/*/edit', array('id' => $model->getId()));
    ...
    public function deleteAction()
    {
        ...
        if( $segmentId )
        {
            try
            {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('advancednewsletter')->__('Segment was successfully deleted'));
            } catch (Exception $ex)
            {
                Mage::getSingleton('adminhtml/session')->addError($ex->getMessage());
            }
