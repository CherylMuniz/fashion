...
class AW_Advancednewsletter_Block_Subscribe extends Mage_Core_Block_Template
{
    ...
    {
        if (Mage::app()->getRequest()->getParam('an_category_id')) $category = Mage::app()->getRequest()->getParam('an_category_id');
        else $category = Mage::registry('current_category') ? Mage::registry('current_category')->getId() : 0;
        return $category;
    }
    ...
    {
        if ($this->getDisplayLabel() == 'false') return false;
        return true;
    }
}