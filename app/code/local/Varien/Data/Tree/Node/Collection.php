...
class Varien_Data_Tree_Node_Collection implements ArrayAccess, IteratorAggregate
{
    ...
    */
	...
	$sortedNodesArray = array();
	$sortedNodes = clone $this;
 
	$bigger = 0;
	foreach ($this->getNodes() as $node) {
		foreach ($this->getNodes() as $compare)
			if (strcmp($node->getName(), $compare->getName()) > 0) $bigger++;
		$sortedNodesArray[$bigger] = $node;
		$bigger = 0;
	}
 
	foreach ($sortedNodes->getNodes() as $node)
		$sortedNodes->delete($node);
 
	ksort($sortedNodesArray);
        
        $j = count($sortedNodesArray);
        $counta = 0;
        for($i = 0; $i <= $j; $i++)
        {
            if (!isset($sortedNodesArray[$i])) break;
            if (($i == 0) || is_int($i/2)) {
                $sortedNodes->add($sortedNodesArray[$counta]);
                $counta++;
            }
            else {
                $sortedNodes->add($sortedNodesArray[intval((($j + $i)/2))]);
            }
        }
	return $sortedNodes;
    }
