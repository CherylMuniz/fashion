    ...
    implements Mage_Catalog_Helper_Product_Configuration_Interface
{
    ...
    public function getCustomOptions(Mage_Catalog_Model_Product_Configuration_Item_Interface $item)
    {
        ...
        $options = array(); $left = $right = $other= array();
        ...
        if ($optionIds) {
            ...
            foreach (explode(',', $optionIds->getValue()) as $optionId) {
                ...
                if ($option) {
                    ...
                    $isLensOpt = false;
                    switch($option->getSku()){
                        case 'sphere_left' : 
                            $left[0] = $tmp; $isLensOpt = true;
                            break;
                        case 'cylinder_left' : 
                            $left[1] = $tmp; $isLensOpt = true;
                            break;
                        case 'axis_left' : 
                            $left[2] = $tmp; $isLensOpt = true;
                            break;
                        case 'nearadd_left' : 
                            $left[3] = $tmp; $isLensOpt = true;
                            break;
                        case 'sphere_right' : 
                            $right[0] = $tmp; $isLensOpt = true;
                            break;
                        case 'cylinder_right' : 
                            $right[1] = $tmp; $isLensOpt = true;
                            break;
                        case 'axis_right' : 
                            $right[2] = $tmp; $isLensOpt = true;
                            break;
                        case 'nearadd_right' : 
                            $right[3] = $tmp; $isLensOpt = true;
                            break;
                        case 'pupil_distance' : 
                            $other[0] = $tmp; $isLensOpt = true;
                            break;
                        case 'lens_for_product' : 
                            $other[1] = $tmp; $isLensOpt = true;
                            break;
                    } if ($isLensOpt) { continue; }
                    // oberig 
                    $options[] = array(
                        'label' => $option->getTitle(),
                        'value' => $group->getFormattedOptionValue($itemOption->getValue()),
            ...
            $options = array_merge($options, $right, $left, $other); // oberig 
        }
