...
*/
class MadCapsule_Dpd_SalesShipmentController extends Mage_Adminhtml_Controller_Sales_Shipment
{
  ...
  public function queueAction()
  {
	...
	if(Mage::getStoreConfig('dpd/misc/servicecode')=='')
	{
		...
		$serviceCode =  strtoupper(Mage::getStoreConfig('dpd/misc/servicecode'));				
	}
	...
	foreach($shipmentIds as $shipmentId)
		{
			...
			$consignment->setServiceId($serviceCode);
			$consignment->setService(Mage::helper('dpd')->niceServiceNames($serviceCode));
			$consignment->setWeight(Mage::helper('dpd')->getWeight($shipmentId,2));
			$consignment->save();
		}
   	
	$this->_getSession()->addSuccess($this->__("".count($shipmentIds)." Consignments queued to be sent to DPD."));
