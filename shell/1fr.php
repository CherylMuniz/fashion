...
class Lenses{
    ...
    public $header_sphere_left = array('SPH (Sphere) left','drop_down','0','10',array(),'sphere_left',);
    public $header_sphere_right = array('SPH (Sphere) right','drop_down','0','11',array(),'sphere_right',);
    ...
    public function getLensIds(){
        $items = Mage::getModel('catalog/product')->getCollection()->addAttributeToFilter('sku', array('varifocal', 'single_vision'));
        $arrIds = null;
        foreach($items as $item){
            $arrIds[] = $item->getId();
        }
